...
const industries = [
  ...
  "Logistics", "EdTech", "Marketplace Apps", "D2C Brands",
];
...
.fg-speedlines { position:absolute; inset:0; overflow:hidden; pointer-events:none; z-index:0; }
.fg-speedline { position:absolute; top:50%; height:1px; background:linear-gradient(90deg,transparent,rgba(20,184,166,.4),transparent); animation:fg-speed 2s ease-in-out infinite; opacity:0; }
...
@keyframes fg-speed { 0%{transform:translateX(0);opacity:0} 10%{opacity:.7} 90%{opacity:.7} 100%{transform:translateX(120vw);opacity:0} }

.fg-hero { display:grid; grid-template-columns:1.1fr 1fr; gap:60px; align-items:center; position:relative; z-index:1; }
.fg-hero-right { display:flex; flex-direction:column; align-items:center; justify-content:center; gap:24px; }
.fg-lottie-hero { width:100%; max-width:340px; }
.fg-big-number { display:inline-block; font-size:clamp(80px,14vw,160px); font-weight:800; line-height:1; background:linear-gradient(135deg,#14b8a6 0%,#22c55e 50%,#2dd4bf 100%); -webkit-background-clip:text; -webkit-text-fill-color:transparent; letter-spacing:-0.06em; margin-bottom:8px; }
.fg-subtitle { font-size:clamp(18px,3vw,26px); font-weight:700; color:var(--text); letter-spacing:-0.02em; margin-bottom:16px; }

/* Mobile hero image */
.fg-hero-img-mobile { display:none; width:100%; border-radius:20px; overflow:hidden; margin-top:16px; }
.fg-hero-img-mobile img { width:100%; display:block; border-radius:20px; max-height:240px; object-fit:cover; }

.fg-drivers { display:grid; grid-template-columns:1fr 1fr; gap:20px; margin-top:40px; }
.fg-driver { padding:32px; border-radius:22px; background:rgba(255,255,255,.04); border:1px solid rgba(255,255,255,.07); transition:.3s var(--ease); position:relative; overflow:hidden; }
.fg-driver::before { content:''; position:absolute; top:0; right:0; width:120px; height:120px; border-radius:0 22px 0 120px; background:linear-gradient(135deg,rgba(20,184,166,.08),transparent); pointer-events:none; }
...
.fg-driver-stat-val { font-size:34px; font-weight:800; background:linear-gradient(135deg,#14b8a6,#22c55e); -webkit-background-clip:text; -webkit-text-fill-color:transparent; line-height:1; }
...
.fg-process-track { position:absolute; top:28px; left:calc(10% + 28px); right:calc(10% + 28px); height:1px; background:linear-gradient(90deg,rgba(20,184,166,.6),rgba(20,184,166,.1)); }
.fg-process-row { display:grid; grid-template-columns:repeat(5,1fr); gap:16px }
.fg-step { text-align:center }
.fg-step-node { width:56px; height:56px; border-radius:50%; margin:0 auto 14px; display:flex; align-items:center; justify-content:center; border:1.5px solid rgba(20,184,166,.4); background:#0c1a16; font-weight:700; font-size:13px; color:#2dd4bf; transition:.3s; }
...
.fg-signals { margin-top:40px; border-radius:22px; overflow:hidden; border:1px solid rgba(255,255,255,.08); }
.fg-signal-header { padding:24px 32px; background:rgba(20,184,166,.08); border-bottom:1px solid rgba(20,184,166,.15); }
...
.fg-signal-item { display:flex; align-items:center; gap:16px; padding:18px 32px; border-bottom:1px solid rgba(255,255,255,.05); font-size:14px; color:var(--muted); transition:.2s; }
.fg-signal-item:last-child { border-bottom:none }
.fg-signal-item:hover { background:rgba(20,184,166,.05); color:var(--text) }
.fg-signal-check { width:20px; height:20px; border-radius:50%; background:rgba(239,68,68,.15); border:1px solid rgba(239,68,68,.3); flex-shrink:0; display:flex; align-items:center; justify-content:center; font-size:10px; color:#f87171; }

.fg-industries { display:flex; flex-wrap:wrap; gap:10px; margin-top:30px }
.fg-industry { padding:9px 16px; border-radius:999px; background:rgba(255,255,255,.05); border:1px solid rgba(255,255,255,.09); font-size:13px; font-weight:500; transition:.2s; }
.fg-industry:hover { background:rgba(20,184,166,.12); border-color:rgba(20,184,166,.4); color:#2dd4bf }

.fg-case { margin-top:40px; display:grid; grid-template-columns:1fr 1.2fr; gap:0; border-radius:24px; overflow:hidden; border:1px solid rgba(255,255,255,.08); }
.fg-case-img { position:relative; min-height:300px }
.fg-case-img img { width:100%; height:100%; object-fit:cover; display:block }
.fg-case-img::after { content:''; position:absolute; inset:0; background:linear-gradient(to right,transparent 40%,rgba(7,16,14,.95)); }
.fg-case-body { padding:44px 40px; background:rgba(255,255,255,.03); display:flex; flex-direction:column; justify-content:center; }
...
.fg-delivery-grid { display:grid; grid-template-columns:repeat(4,1fr); gap:16px; margin-top:40px; }
.fg-delivery-card { padding:28px 24px; border-radius:20px; background:rgba(255,255,255,.04); border:1px solid rgba(255,255,255,.07); transition:.25s; }
.fg-delivery-card:hover { border-color:rgba(20,184,166,.3) }
.fg-delivery-num { font-size:48px; font-weight:800; line-height:1; color:rgba(20,184,166,.18); margin-bottom:16px; letter-spacing:-0.04em; }
.fg-delivery-card h3 { margin:0 0 10px; font-size:16px; font-weight:700 }
.fg-delivery-card p { margin:0; font-size:13px; color:var(--muted); line-height:1.65 }

@media(max-width:1000px) {
  .fg-hero { grid-template-columns:1fr; gap:32px }
  .fg-hero-right { display:none }
  .fg-hero-img-mobile { display:block }
  .fg-drivers { grid-template-columns:1fr }
  .fg-case { grid-template-columns:1fr }
  .fg-case-img { min-height:240px }
  .fg-case-img::after { background:linear-gradient(to bottom,transparent 40%,rgba(7,16,14,.95)); }
  .fg-case-body { padding:28px 24px }
  .fg-delivery-grid { grid-template-columns:1fr 1fr }
  .fg-signal-header { padding:20px 20px }
  .fg-signal-item { padding:16px 20px }
}
@media(max-width:700px) {
  .fg-process-row { grid-template-columns:1fr 1fr }
  .fg-process-track { display:none }
  .fg-delivery-grid { grid-template-columns:1fr }
  .fg-case-body { padding:20px 16px }
  .fg-case-body h2 { font-size:18px }
}
`;
    ...
    <div>
      ...
      <section className="section" style={{ overflow: "hidden", position: "relative" }}>
        ...
        <div className="container fg-hero">
          <div>
            ...
            <div className="fg-hero-img-mobile reveal">
              <img
          ...
          <div className="fg-hero-right reveal">
            <div className="fg-lottie-hero">
              <lottie-player autoplay loop mode="normal" src="https://assets4.lottiefiles.com/packages/lf20_raiw2hpe.json" />
            </div>
          </div>
        </div>
      </section>
      ...
      <section className="section">
        <div className="container">
          ...
          <div className="fg-industries reveal">
            {industries.map(i => <div key={i} className="fg-industry">{i}</div>)}
          </div>
        </div>
      </section>
      ...
      <section className="section section--alt">
        <div className="container">
          ...
          <div className="fg-delivery-grid reveal">
            {deliverySteps.map((step) => (
              <div
                key={step.n}
                className="fg-delivery-card"
                onMouseEnter={e => e.currentTarget.style.borderColor = "rgba(20,184,166,.3)"}
                onMouseLeave={e => e.currentTarget.style.borderColor = "rgba(255,255,255,.07)"}
              >
                <div className="fg-delivery-num">{step.n}</div>
                <h3>{step.title}</h3>
                <p>{step.desc}</p>
              </div>
            ))}
      ...
      <section className="section">
        <div className="container">
          <div className="cta-band reveal">
            ...
            <Link className="btn cta-band__btn" to="/contact">Book a velocity audit</Link>
          </div>
        </div>
      </section>
    </div>
